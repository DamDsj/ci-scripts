...
if [ "$BUILDTYPE" == "tests" ]; then
    PDEBUILDTYPE=I
    REPONAME="Turnus Orcc profiler tests repository"
    NBUILDSTOKEEP=3
else
    PDEBUILDTYPE=R
    REPONAME="Turnus Orcc profiler release"
...
TURNUS_MAINFEATURE_ID="co.turnus.profiler.orcc.sim"
ARTIFACTSFOLDER="final_artifacts"

echo "plugin path: $ECLIPSE_BUILD:$BUILDDIR" 
...
$P2ADMIN -application org.eclipselabs.equinox.p2.composite.repository \
    -location file:$(pwd) \
    -add $(echo ${TURNUS_MAINFEATURE_ID}_* | sed -e "s/ /,/g") \
...
echo ""
echo "****************************************************************"
echo "*     Installs Turnus Orcc plugins into eclipse runtime        *"
echo "****************************************************************"
